...
namespace Pinaka.Core
{
    public class PinakaParameters
    {
        ...
        private static string SanitizeParameter(string parameterNameOrValue)
        {
            parameterNameOrValue = parameterNameOrValue.TrimStart(new char[] { '-' });
            if (parameterNameOrValue.StartsWith("\"") && parameterNameOrValue.EndsWith("\"") || parameterNameOrValue.StartsWith("'") && parameterNameOrValue.EndsWith("'"))
            {
                parameterNameOrValue = parameterNameOrValue.Substring(1, parameterNameOrValue.Length - 2);
            }
            return parameterNameOrValue.ToLower();
        }
        ...
        public static PinakaParameters Parse(string[] args)
        {
            ...
            foreach (var arg in args)
            {
                ...
                if (parts.Length == 2)
                {
                    arguments.Add(SanitizeParameter(parts[0]), SanitizeParameter(parts[1]));
                }
                // If not, assume it's just a named argument without a value
                else
                {
                    arguments.Add(SanitizeParameter(arg), null);
                }
            }
            RawValues = arguments;
